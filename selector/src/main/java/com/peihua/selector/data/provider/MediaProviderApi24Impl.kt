...
internal class MediaProviderApi24Impl(context: Context) : IMediaProvider(context) {
    ...
    private fun queryMedia(
        ...
        if (DEBUG) {
            Log.d(TAG, ("queryMedia() uri=" + uri + " cat=" + category + " mimeTypes=" + mimeTypes.contentToString() + " limit=" + config.pageSize))
        }
        ...
        try {
            return context.contentResolver.acquireUnstableContentProviderClient(MediaStore.AUTHORITY).use { client ->
                if (client == null) {
                    Log.e(TAG, "Unable to acquire unstable content provider for " + MediaStore.AUTHORITY)
                    return null
                }
            ...
            Log.e(TAG, ("Failed to query merged media with extras: $extras"), ignored)
            return null
            ...
            if (DEBUG) {
                if (result == null) {
                    ...
                    Log.d(TAG, "queryMedia() loaded " + result.count + " items")
                    if (DEBUG_DUMP_CURSORS) {
                        Log.v(TAG, DatabaseUtils.dumpCursorToString(result))
                    }
                }
            }
        }
    ...
    companion object {
        ...
        private const val DEBUG_DUMP_CURSORS: Boolean = false
        private const val TAG: String = "MediaProviderApi24Impl"
    }
}