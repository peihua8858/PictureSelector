...
public class StringUtils {
  ...
  public static String getICUFormatString(Resources resources, int count, int resourceID) {
    MessageFormat msgFormat = new MessageFormat(resources.getString(resourceID));
  ...
  public static List<String> verifySupportedUncachedRelativePaths(List<String> unverifiedPaths) {
      ...
      for (final String path : unverifiedPaths) {
          ...
          if (path.startsWith("/")) {
              Log.w("StringUtils", "Relative path config must not start with '/'. Ignoring: " + path);
              continue;
          }
          if (!path.endsWith("/")) {
              Log.w("StringUtils", "Relative path config must end with '/'. Ignoring: " + path);
              continue;
          }
